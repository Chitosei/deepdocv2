...
class OCR(object):
    ...
    def get_rotate_crop_image(self, img, points):
        """
        Crop image based on the given points.
        """
        assert len(points) == 4, "Shape of points must be 4*2"
        img_crop_width = int(
            max(
...
class VietnameseOCR:
    ...
    def extract_text_from_image(self, image):
        ...
        return pytesseract.image_to_string(preprocessed_image, lang="vie").strip()

    def correct_text(self, extracted_text):
        """Correct the OCR output using Hugging Face model"""
        try:
            prediction = self.corrector(extracted_text, max_length=1024)
